...
class Window:
	canvas = None
	var = None	#indicator of one of the three modes.
	root = None
	selected = None
# 	popup = None
	transforms = {}
	...
	def on_motion_handler(self, event):
		if self.selected != None:
			coord = self.canvas.bbox( self.selected )
			ori_x, ori_y = (coord[0]+coord[2])/2, (coord[1]+coord[3])/2
	...
	def	redraw_bezier_curve(self):
		...
		for t in range(0, 101):
			...
			ps = ps + [p[0], p[1]]	
		self.canvas.create_line(ps, smooth=True, tags='original_bezier')
	...
	def redraw_handle_affected_curve(self):
		...
		self.canvas.create_line(tps, smooth=True, fill='magenta', tags='handle_affected')
	...
	def redraw_approximated_bezier(self):
		...
		for t in range(0, 101):
			p = np.dot( np.asarray( [(float(t)/100)**3, (float(t)/100)**2, float(t)/100, 1] ), known )
			ps = ps + [p[0], p[1]]	
		self.canvas.create_line(ps, smooth=True, fill='green', tags='approximated')	 
	...
	def init_menubar(self, menubar):
		...
		self.var = IntVar()
		mb_edit.menu.add_radiobutton(label='add control point', variable=self.var, value=0, command=self.change_mode)
		mb_edit.menu.add_radiobutton(label='add handle', variable=self.var, value=1, command=self.change_mode)
		mb_edit.menu.add_radiobutton(label='edit handle', variable=self.var, value=2, command=self.change_mode)
	...
	def popup_handle_editor(self, handle_id):
		...
		popup = self.canvas.create_window((coord[0]+coord[2])/2, (coord[1]+coord[3])/2, anchor=NW, width=204, height=140, window=frame)	
